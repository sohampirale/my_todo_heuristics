'use client';

import { InputHTMLAttributes, forwardRef } from 'react';
import { isSeedEnabled } from '@/lib/seeds';

interface SeededToggleProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> {
  seedPage: string;
  label: string;
  description?: string;
}

export const SeededToggle = forwardRef<HTMLInputElement, SeededToggleProps>(
  ({ seedPage, label, description, checked = false, onChange, ...props }, ref) => {
    const ambiguousLabels = isSeedEnabled(seedPage, 'ambiguous_toggle_labels');
    const noWarn = isSeedEnabled(seedPage, 'destructive_toggle_no_warn');
    
    const displayLabel = ambiguousLabels ? 'Toggle' : label;
    const displayDescription = ambiguousLabels ? 'Enable/Disable' : description;

    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (noWarn && e.target.checked && label.toLowerCase().includes('delete')) {
        // No warning for destructive action
        onChange?.(e);
      } else {
        onChange?.(e);
      }
    };

    return (
      <div className="flex items-center justify-between py-4">
        <div>
          <p className="text-sm font-medium text-gray-900">{displayLabel}</p>
          {displayDescription && (
            <p className="text-sm text-gray-500">{displayDescription}</p>
          )}
        </div>
        <label className="relative inline-flex items-center cursor-pointer">
          <input
            ref={ref}
            type="checkbox"
            checked={checked}
            onChange={handleChange}
            className="sr-only peer"
            {...props}
          />
          <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
        </label>
      </div>
    );
  }
);

SeededToggle.displayName = 'SeededToggle';
